...
// Addition test quiz module for addition problems with numbers >= 10
export default class AdditionTestQuiz extends QuizBase {
    ...
    async startQuiz() {
        try {
            ...
            this.currentQuizType = 'addition-test';

            // Fetch new session from API
            const response = await fetch('/session/addition-test', {
                method: 'GET',
                headers: {
            ...
            if (button) {
                ...
                button.innerHTML = originalHTML || this.getDefaultButtonHTML('addition-test');
            }
        }
    }
